...
def functionlogs(log='app'):
    def wrap(function):
        ...
        def wrapper(*args, **kwargs):
            ...
            logger.debug("*******Starting function %s : %s" % (file_name, function.__name__))
            ...
            logger.debug("*******Finished function %s : %s in %s seconds" % (file_name, function.__name__,end_time-init_time))
            return response
        return wrapper
    return wrap
